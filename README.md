...
Current Version: **sameersbn/redmine:7.0.0-3**
...
```bash
docker pull sameersbn/redmine:7.0.0-3
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0-3 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0-3 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0-3 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0-3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0-3 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:7.0.0-3
```
